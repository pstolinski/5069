...
void flywheelController(int amountRPM){//essentially your flywheel rpm control, 0 is coast to stop, anything more is spool up
  if(amountRPM == 0){
      flywheelBottom.stop();
      flywheelTop.stop();
...
void verifyLaunchIntegrity(int requiredRPM){//verify flywheel is of adequite revolutions per minute and then fire the pneumatic
...
void autonomous(void) {
  ...
  autoTurn(left, -180, 100);

  //flapPneumatic.off();
  autoTurn(right, 13, 75);
  flywheelController(160);
  verifyLaunchIntegrity(2800);
  verifyLaunchIntegrity(2800);
...
void usercontrol(void) {
  ...
  while (true) {
    ...
    float goalX = botX - 500;
    float goalY = botY - 500;

    goalX1 = goalX;
    goalY1 = goalY;

    float turnAngle = (std::atan(goalX / goalY) * 57.2957795)+180;
...
if(Controller1.Axis2.position(percent) > 20){
  if(Controller1.Axis2.position(percent) > 75){
    flywheelSpeed = flywheelSpeed + 10;
  }else{
  flywheelSpeed = flywheelSpeed + 5;
  }
  waitUntil(Controller1.Axis2.position(percent) < 20 && Controller1.Axis2.position(percent) > -20);
}else if(Controller1.Axis2.position(percent) < -20){
  if(Controller1.Axis2.position(percent) < -75){
    flywheelSpeed = flywheelSpeed - 10;
  }else{
  flywheelSpeed = flywheelSpeed - 5;
  ...
  waitUntil(Controller1.Axis2.position(percent) < 20 && Controller1.Axis2.position(percent) > -20);
    ...
    Brain.Screen.print("Top Motor RPM: ");
    Brain.Screen.print(flywheelTop.velocity(rpm));
    Brain.Screen.setCursor(2,1);
    Brain.Screen.print("Bottom Motor RPM: ");
    Brain.Screen.print(flywheelBottom.velocity(rpm));
    Brain.Screen.setCursor(3,1);
    Brain.Screen.print("Rotation RPM: ");
    Brain.Screen.print(flywheelRotation.velocity(rpm));
    Controller1.Screen.clearLine(1);
    Controller1.Screen.setCursor(1,1);
    Controller1.Screen.print(flywheelSpeed);
    Controller1.Screen.print(" : ");
    Controller1.Screen.print(flywheelToggle);
